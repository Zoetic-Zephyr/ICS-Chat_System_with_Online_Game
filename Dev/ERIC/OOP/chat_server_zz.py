...
#==============================================================================
                ...
                if msg[1:2] == M_GAME:
                    ...
                    if self.group.is_member(info):
                        opponent = info
                        self.opponent_dic[from_name] = opponent
                else:
                    # Finding the list of people to send to
                    the_guys = self.group.list_me(from_name)  # [1:]
                    sentence = msg[1:]
                    # sentences = text_proc(said, from_name)
                    self.indices[from_name].add_msg_and_index(sentence)
                    for g in the_guys[1:]:
                        to_sock = self.logged_name2sock[g]
                        # self.indices[g].add_msg_and_index(sentences)
                        # mysend(to_sock, "...Remember to index the messages before sending, or search won't work")
                        mysend(to_sock, M_EXCHANGE + sentence)
            elif code == M_GAME:
                from_name = self.logged_sock2name[from_sock]
                barpos_x = msg[1:]
                print(barpos_x)
                to_sock = self.logged_name2sock[sekf.opponent_dic[from_name]]
...
#==============================================================================
                ...
                poem_indx = int(msg[1:])
                from_name = self.logged_sock2name[from_sock]
                # note: the poem retrive from the sonnet is a list of
                # strings
                poem = []
                try:
...
#==============================================================================
                ...
                if len(search_rslt) == 0:
                    search_rslt.append('Message not found!')
