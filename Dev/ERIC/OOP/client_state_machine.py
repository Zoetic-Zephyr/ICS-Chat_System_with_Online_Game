...
class ClientSM:
    ...
    def run_game_client(self):
        ...
        self.player[self.me] = breakout_simple.Player()
        allsprites.add(self.player[self.me])
        self.player[self.opponent] = breakout_simple.Player()
        allsprites.add(self.player[self.opponent])
        # Create the ball
        ball = breakout_simple.Ball()
        allsprites.add(ball)
        balls.add(ball)
        '''
        ...
        while not exit_program:
            ...
            clock.tick(30)
            # Clear the screen
            screen.fill(breakout_simple.black)
            ...
            if not game_over:
                ...
                self.player[self.me].update()
                mysend(self.s, M_GAME + str(self.player[self.me].rect.x))
                got = myrecv(self.s)[1:]
                print(got)
                self.player[self.opponent].rect.x = int(got)
                # self.opponent.update()
                # game_over = ball.update()

            # If we are done, print game over
            if game_over:
                text = font.render("Game Over", True, breakout_simple.white)
                textpos = text.get_rect(centerx=background.get_width() / 2)
                textpos.top = 300
                screen.blit(text, textpos)

            # See if the ball hits the player paddle
            if pygame.sprite.spritecollide(self.player[self.me], balls, False):
                # The 'diff' lets you try to bounce the ball left or right
                # depending where on the paddle you hit it
                diff1 = (self.player[self.me].rect.x + self.player[self.me].width / 2) - \
                    (ball.rect.x + ball.width / 2)

                # Set the ball's y position in case
                # we hit the ball on the edge of the paddle
                ball.rect.y = screen.get_height() - self.player[self.me].rect.height - ball.rect.height - 1
                ball.bounce(diff1)

            # See if the ball hits the player paddle
            if pygame.sprite.spritecollide(self.player[self.opponent], balls, False):
                # The 'diff' lets you try to bounce the ball left or right
                # depending where on the paddle you hit it
                diff2 = (self.player[self.opponent].rect.x + self.player[self.opponent].width / 2) - \
                    (ball.rect.x + ball.width / 2)

                # Set the ball's y position in case
                # we hit the ball on the edge of the paddle
                ball.rect.y = screen.get_height() - self.player[self.opponent].rect.height - ball.rect.height - 1
                ball.bounce(diff2)
            '''
